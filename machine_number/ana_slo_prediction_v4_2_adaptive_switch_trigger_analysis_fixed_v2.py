...
ANALYSIS_ROOT = Path(
    r"C:\Users\user\Desktop\Documents\SlotAnalyzer\data\maruhan_maebashi\machine_number\analysis_31days_deep"
    r"\machine_number\analysis\_31days\_deep"
)

# 螳滄圀縺ｮVer.4.2 long-term adaptive縺ｮ蜃ｺ蜉帙ヱ繧ｹ縲・INPUT_DAILY = (
    ANALYSIS_ROOT
    / "39"
    / "Ver4"
    / "2"
...
# 迺ｰ蠅・↓繧医▲縺ｦ繝輔か繝ｫ繝蜷阪′逡ｰ縺ｪ繧句ｴ蜷医↓蛯吶∴縺溯・蜍墓爾邏｢縲・def resolve_input_daily():
    if INPUT_DAILY.exists():
        return INPUT_DAILY

    candidates = []

    # 譌｢遏･縺ｮ螳溷・蜉帙ヱ繧ｿ繝ｼ繝ｳ繧貞・蟶ｰ謗｢邏｢縲・    for p in ANALYSIS_ROOT.rglob(
        "39_Ver4_2_long_term_adaptive_rank_band_fixed_daily.csv"
    ):
        candidates.append(p)

    # 蠢ｵ縺ｮ縺溘ａ繝輔ぃ繧､繝ｫ蜷阪・陦ｨ險俶昭繧後ｂ謗｢邏｢縲・    if not candidates:
        for p in ANALYSIS_ROOT.rglob("*adaptive*rank*band*fixed*daily.csv"):
            candidates.append(p)

    if not candidates:
        raise FileNotFoundError(
            "Ver.4.2 long-term adaptive daily CSV 縺瑚ｦ九▽縺九ｊ縺ｾ縺帙ｓ縲・n"
            f"讀懃ｴ｢繝ｫ繝ｼ繝・ {ANALYSIS_ROOT}\n"
            "譛溷ｾ・ヵ繧｡繧､繝ｫ蜷・ "
            "39_Ver4_2_long_term_adaptive_rank_band_fixed_daily.csv"
        )

    # 39/Ver4/2/long/term/adaptive/rank/band/fixed 縺ｫ譛繧りｿ代＞繧ゅ・繧貞━蜈医・    def score(p):
...
def load_daily():
    input_daily = resolve_input_daily()
