...
def analyze_machine(df):
    ...
    out = g.agg(
        ...
        勝率=("win", "mean"),
        平均G数=("games", "mean"),
        +1000枚率=("strong_1000", "mean"),
        +2000枚率=("strong_2000", "mean"),
        +3000枚率=("strong_3000", "mean"),
        -1000枚率=("loss_1000", "mean"),
    ).reset_index()

    # 基礎スコア：平均差枚、勝率、+1000率を標準化して合成。
    # 過度に単一指標へ依存しないための探索用スコア。
    z_diff = (out["平均差枚"] - out["平均差枚"].mean()) / (safe_std(out["平均差枚"]) or 1)
    z_win = (out["勝率"] - out["勝率"].mean()) / (safe_std(out["勝率"]) or 1)
    z_1000 = (out["+1000枚率"] - out["+1000枚率"].mean()) / (safe_std(out["+1000枚率"]) or 1)
...
def analyze_previous_next(df):
    ...
    summary = (
        ...
        .agg(
            ...
            翌日+1000率=("翌日差枚", lambda s: (s >= 1000).mean()),
        )
        .reset_index()
    )
    ...
    by_machine = (
        ...
        .agg(
            ...
            前日大幅マイナス翌日+1000率=("翌日+1000", lambda s: s[detail.loc[s.index, "前日大幅マイナス"]].mean()
                                      if (detail.loc[s.index, "前日大幅マイナス"]).any() else float("nan")),
...
def analyze_special_days(df):
    ...
    daily = (
        ...
        .agg(
            ...
            平均差枚=("diff", "mean"),
            勝率=("win", "mean"),
            +1000枚台数=("strong_1000", "sum"),
            +2000枚台数=("strong_2000", "sum"),
            +3000枚台数=("strong_3000", "sum"),
            平均G数=("games", "mean"),
        )
        .reset_index()
    )
    ...
    # 店全体の基準から偏差化
    for col in ["平均差枚", "勝率", "+1000枚台数", "+2000枚台数", "+3000枚台数"]:
...
def main():
    ...
    print(
        special[
            ["date", "平均差枚", "勝率", "+1000枚台数", "+2000枚台数", "+3000枚台数",
             "特定日スコア", "候補ランク"]
