...
APPROVAL_TOOL = "MARUHAN_INVENTORY_GUARD_APPROVAL_V1"
...
def approve_maruhan_inventory_incident(
    data_dir: Path,
    previous_date: date,
    change_date: date,
    reason: str,
    *,
    ...
    if state.get("store") != MARUHAN_MAEBASHI_POLICY.store:
        raise RuntimeError(
    ...
    evidence = inspect_inventory_transition(
        MARUHAN_MAEBASHI_POLICY.store,
        previous_path,
        current_path,
        previous_date,
        change_date,
        policy_version="MARUHAN_APPROVAL_V1",
    ...
    if evidence.previous_machine_count != EXPECTED_MACHINE_COUNT:
        raise RuntimeError(
            "Previous inventory is outside the validated Maruhan baseline. "
    ...
    guard_result = assess_inventory_guard(
        data_dir,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Safely approve one exact Maruhan Maebashi Inventory Guard "
    ...
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=ROOT / "data" / "maruhan_maebashi" / "machine_number",
    )
    parser.add_argument(
        "--previous-date",
        required=True,
