...
ANALYSIS_ROOT = Path(
    r"C:\Users\user\Desktop\Documents\SlotAnalyzer\data\maruhan_maebashi"
    r"\machine_number\analysis_31days_deep"
)

# 実際のVer.4.2 long-term adaptiveの出力パス。
INPUT_DAILY = (
    ANALYSIS_ROOT
    / "39_Ver4_2_long_term_adaptive_rank_band_fixed_daily.csv"
)

# 環境によってフォルダ名が異なる場合に備えた自動探索。
def resolve_input_daily():
    ...
    if not candidates:
        raise FileNotFoundError(
            "Ver.4.2 long-term adaptive daily CSV が見つかりません。\n"
            f"検索ルート: {ANALYSIS_ROOT}\n"
            "期待ファイル名: "
            "39_Ver4_2_long_term_adaptive_rank_band_fixed_daily.csv"
        )

    # 39/Ver4/2/long/term/adaptive/rank/band/fixed に最も近いものを優先。
    def score(p):
...
OUT_DIR = Path(
    r"C:\Users\user\Desktop\Documents\SlotAnalyzer\data\maruhan_maebashi"
    r"\machine_number\analysis_31days_deep\45\_Ver4_2_adaptive_switch_trigger"
)
...
def add_prior_features(block_table):
    ...
    for block_no in sorted(block_table["block"].unique()):
        ...
        row = {
            "block": block_no,
            "block_start": current["block_start"].min(),
            "block_end": current["block_end"].max(),
