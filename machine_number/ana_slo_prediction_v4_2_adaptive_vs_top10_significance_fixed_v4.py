...
def load_daily():
    ...
    # Actual schema of the long-term validation output:
    # date, block, block_start, block_end, rule, selected_rule,
    # is_adaptive_selection, machines, avg_diff, ..., total_diff
    required = [
        "date",
        "block",
        "rule",
    ...
    # Normalize boolean values robustly.
...
def select_comparable_days(df):
    """
    Compare the actual adaptive-selected rule against fixed TOP10 on the
    same dates.  The long-term daily CSV already contains
    is_adaptive_selection, so no reconstruction from a separate file is
    necessary.
    """
    ...
    if adaptive.empty:
        raise RuntimeError(
            "is_adaptive_selection=True のAdaptive行が見つかりません。"
        )

    # There should be one selected adaptive rule per block/date.
    adaptive_daily = (
    ...
    paired["improvement"] = (
        paired["adaptive_avg_diff"] - paired["top10_avg_diff"]
    )
    paired["total_improvement"] = (
        paired["adaptive_total_diff"] - paired["top10_total_diff"]
    )
