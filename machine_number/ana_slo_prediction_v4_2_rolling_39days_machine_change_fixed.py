...
OUT_DIR = (
    ...
    / "49_Ver4_2_rolling_39days_machine_change"
)
...
def build_features(
    ...
    """
    Build an OOS feature panel for every machine present on target_date.

    Machine-change handling:
    - The target day's machine_name is authoritative.
    - If the latest historical machine_name at the same machine_no differs,
      machine_changed is set to 1.
    - Machine-specific history from a previous machine type is not inherited.
    - When the current machine type has history at the same machine_no,
      only that history is used.
    - When no same-machine/current-type history exists, conservative
      pre-target store-level priors are used.
    - type_avg may use pre-target history for the current machine type
      across the store.
    """
    ...
    # Store-level priors use only data before target_date.
    store_avg = float(
        hist["diff"].mean()
    )
    ...
    store_plus1000 = float(
        hist["plus1000"].mean()
    )

    store_plus2000 = float(
        hist["plus2000"].mean()
    )
    ...
    # Start from target-day machines so newly installed machines
    # are not silently dropped by the merge.
    for row in actual.itertuples(
        ...
        if (
            ...
            # Critical:
            # never inherit old machine-type history.
            m = all_no_hist[
                all_no_hist[
                    "machine_name"
                ]
        ...
        if history_n > 0:
            ...
            # New/current machine type has no history at this machine_no.
            # Use neutral pre-target priors rather than the old machine.
            avg31 = store_avg
            ...
            plus1000_rate = store_plus1000
            plus2000_rate = store_plus2000
            bounce_signal = 0.0
...
def main():
    ...
    print("=" * 70)
    print(
        "Ana-Slo Ver.4.2 Rolling Walk-Forward OOS Test - Machine Change Experiment"
    )
    print("=" * 70)
    ...
    out_daily = (
        OUT_DIR
        / "49_Ver4_2_rolling_39days_machine_change_daily.csv"
    )

    out_summary = (
        OUT_DIR
        / "49_Ver4_2_rolling_39days_machine_change_summary.csv"
    )

    out_ranking = (
        OUT_DIR
        / "49_Ver4_2_rolling_39days_machine_change_ranking.csv"
    )

    out_top10 = (
        OUT_DIR
        / "49_Ver4_2_rolling_39days_machine_change_top10_compare.csv"
    )

    out_topn = (
        OUT_DIR
        / "49_Ver4_2_rolling_39days_machine_change_model_topn.csv"
    )
    ...
    print()
    print(
        "Ver.4.2 rolling OOS machine-change experiment complete."
    )
