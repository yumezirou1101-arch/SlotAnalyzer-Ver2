...
DATA_START = pd.Timestamp("2026-07-11")
...
def load_data():
    ...
    for p in (DAY11_FILE, MERGED_FILE):
        if p.exists():
            print(f"Loading: {p}")
    ...
    no_col = find_col(
        df,
        ["machine_no", "machine_number", "台番号", "台番", "台No", "台NO"],
    )
    name_col = find_col(
        df,
        ["machine_name", "machine", "機種名", "機種"],
    )
    diff_col = find_col(
        df,
        ["diff", "差枚", "差枚数", "差枚数(枚)"],
...
def adaptive_main():
    ...
    diagnostic["adaptive_avg_diff"] = [
        float(adaptive_blocks.loc[
            adaptive_blocks["block"] == b, "avg_diff"
        ].iloc[0])
        for b in diagnostic["block"]
    ]
    ...
    out_dir = OUT_DIR
    out_daily = out_dir / "37_Ver4_2_adaptive_rank_band_daily.csv"
    out_block = out_dir / "37_Ver4_2_adaptive_rank_band_blocks.csv"
    out_selection = out_dir / "37_Ver4_2_adaptive_rank_band_selection.csv"
    out_summary = out_dir / "37_Ver4_2_adaptive_rank_band_summary.csv"
    out_diag = out_dir / "37_Ver4_2_adaptive_rank_band_diagnostic.csv"
    ...
    print(out_diag)
    print()
    print("Ver.4.2 adaptive rank band walk-forward complete.")
