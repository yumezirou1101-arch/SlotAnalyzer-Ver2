...
# True One-Click Daily Update V1
# ============================================================
#
# Flow
# ----
...
#
# 3) Update Champion / Challenger forward tracking
#    - ana_slo_prediction_v4_2_forward_champion_challenger.py
#
# 4) Build live predictions
...
def main() -> None:
    ...
    if not args.skip_fetch:
        ...
        print()
        print(
            "STEP 1 / 4 - FETCH NEWEST HTML : SKIPPED"
        )
    ...
    # --------------------------------------------------------
    elapsed = run_stage(
        "STEP 2 / 4 - HTML TO VALIDATED DAILY CSV",
        SCRIPT_CONVERT,
        [
            str(
                source_html
    ...
    # --------------------------------------------------------
    # 3) 63 FORWARD TRACKING
    # --------------------------------------------------------
    elapsed = run_stage(
        "STEP 3 / 4 - CHAMPION / CHALLENGER FORWARD",
        SCRIPT_FORWARD,
    )
    ...
    elapsed = run_stage(
        "STEP 4 / 4 - LIVE PREDICTION PIPELINE 79",
        SCRIPT_LIVE,
        live_args,
    )
