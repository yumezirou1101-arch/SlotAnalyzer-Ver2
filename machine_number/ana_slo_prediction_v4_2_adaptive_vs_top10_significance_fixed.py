...
if ANALYSIS_ROOT.exists():
    ...
    if daily_candidates:
        INPUT_DAILY = daily_candidates[-1]

    if selection_candidates:
        INPUT_SELECTION = selection_candidates[-1]

if INPUT_DAILY is None:
    # Fallback: tolerate alternate naming/path variants containing the same
    # distinctive 39-series identifiers.
    if ANALYSIS_ROOT.exists():
        candidates = sorted(
            p for p in ANALYSIS_ROOT.rglob("*39*adaptive*rank*band*fixed*daily*.csv")
            if p.is_file()
        )
        if candidates:
            INPUT_DAILY = candidates[-1]

if INPUT_SELECTION is None and ANALYSIS_ROOT.exists():
    candidates = sorted(
        p for p in ANALYSIS_ROOT.rglob("*39*adaptive*rank*band*fixed*selection*.csv")
        if p.is_file()
    )
    if candidates:
