...
async def capture_target_response(
    ...
    if captured["body"] is None:
        ...
        try:
            ...
            print()
            print(
                f"手動確認タブURL: {current_url}"
            )

            dom_html = await (
                candidate_page.content()
            )

            print(
                f"手動DOM HTML文字数: {len(dom_html):,}"
            )
            ...
            if len(dom_html) < 10000:
                ...
                print()
                print(
                    "[成功] 手動表示されたChrome DOMからHTMLを取得しました。"
                )
