    ...
    if captured["body"] is None:
        ...
        for page_index, candidate_page in enumerate(
            ...
            try:
                ...
                print()
                print(
                    f"タブ候補 {page_index}/{len(candidate_pages)}: {current_url}"
                )
                ...
                for frame_index, frame in enumerate(
                    ...
                    try:
                        ...
                        # extractor互換のため、IDが違う場合は
                        # all_data_table に正規化する。
                        soup = BeautifulSoup(
                            table_html,
                            "html.parser"
                        )
                        ...
                        normalized_table_html = str(
                            table_tag
                        )

                        # frame内に表があっても、titleは親ページから補う。
                        # get_page_date / check_store / extractor を壊さないため
                        # 最小の完全HTMLを組み立てる。
                        synthetic_html = (
                            "<!DOCTYPE html>"
                            "<html>"
                            ...
                            "</head>"
                            "<body>"
                            f"{normalized_table_html}"
                            "</body>"
                            "</html>"
                        )

                        print(
                            f"    台データ表候補: rows={row_count:,}"
                        )

                        if row_count > best_rows:

                            best_rows = row_count
                            best_html = synthetic_html
                            best_url = current_url

                    except Exception as e:

                        print(
                            "    [注意] このframeは読み取れませんでした。"
                        )
                        print(
                            f"    {e}"
                        )
        ...
        if (
            ...
            print()
            print(
                "[成功] Chromeのframe内から台データ表を取得しました。"
            )
            print(
                f"採用テーブル行数: {best_rows:,}"
            )
            print(
            ...
            print()
            print(
                "[警告] 台データ表をDOM/frameから取得できませんでした。"
            )
            print(
                "次は表の実装方式（iframe / shadow DOM / 仮想DOM）を診断します。"
            )
