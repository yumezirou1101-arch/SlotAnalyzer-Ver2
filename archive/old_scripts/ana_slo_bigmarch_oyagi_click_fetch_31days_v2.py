...
# ============================================================
# Big March Takasaki Oyagi - Multi-Day Link-Click Fetch V2
# ============================================================
#
# Main change from V1:
# - Historical machine counts are allowed to vary by date.
# - "276 machines exactly" is NO LONGER required.
#
# A daily page is accepted only when:
# - Store name matches
# - Target date matches
# - Main machine table exists
# - Required columns exist
# - Machine count is >= --min-machines
# - Machine numbers are unique
# - Machine name has no missing/blank rows
# - G and diff are numeric
# - G is not negative
#
# Existing HTML files are skipped unless --overwrite is used.
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=(
            f"Maximum number of date links to process. "
    ...
    parser.add_argument(
        ...
        help=(
            f"Minimum acceptable machine rows for a historical page. "
            f"Default: {DEFAULT_MIN_MACHINES}."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Overwrite existing source HTML files. "
...
async def ensure_list_page(page):
    ...
    if (
        LIST_TITLE_TEXT in title
        and has_store_text(title)
    ):
        return

    print(
        "returning to list page : browser back"
    )
...
async def collect_date_links(page):
    ...
    for i in range(count):
        ...
        text_match = DATE_TEXT_RE.match(
            text
        )

        url_match = DATE_URL_RE.search(
            href
        )
...
def save_log(
    ...
    path = (
        LOG_DIR
        / f"bigmarch_oyagi_fetch_v2_{stamp}.csv"
    )
    ...
    async with async_playwright() as p:
        ...
        for pos, item in enumerate(
            ...
            try:
                ...
                print(
                    f"machine count check   : {result['machine_count_ok']}"
                )
        ...
        print()
        print(
            "Historical machine count changes are allowed."
        )

        print(
            f"Pages with fewer than {args.min_machines} machines are rejected."
        )

        print(
            "Duplicate machine numbers or invalid G/diff values are rejected."
        )

        print(
            "Existing source HTML is skipped unless --overwrite is specified."
        )
