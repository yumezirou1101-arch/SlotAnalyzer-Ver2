...
async def capture_target_response(
    ...
    if captured["body"] is None:
        ...
        print()
        print(
            "Network ResponseでHTMLを取得できなかったため、"
        )
        print(
            "現在Chromeに表示されているDOMを確認します。"
        )

        try:

            # JavaScript描画やCloudflare等の画面遷移が落ち着く余地を与える
            ...
            # 短すぎるHTML（403画面、エラーページ等）は採用しない
            if len(dom_html) < 10000:

                print()
                print(
                    "[警告] DOM HTMLが短すぎるため採用しません。"
                )

            else:

                captured["body"] = dom_html.encode(
                    "utf-8"
                )
                captured["status"] = "DOM_FALLBACK"
                captured["url"] = current_url

                print()
                print(
                    "[成功] Chrome DOMからHTMLを取得しました。"
                )
