...
async def capture_target_response(
    ...
    if captured["body"] is None:
        ...
        print(
            "重要: できれば同じタブではなく、9222付きChromeの新しいタブで開いてください。"
        )
        print(
            "対象ページを完全表示したあと、PowerShellに戻って Enter を押してください。"
        )
        print()
        ...
        if not matching_pages:
            ...
            print()
            print(
                "[警告] 対象URLを表示している生きたChromeタブが見つかりません。"
            )
            print(
                "9222付きChromeの新しいタブで対象ページを開いてください。"
            )

        # 候補タブをすべて試す。
        # 403で壊れた古いタブが残っていても、正常な新規タブを拾える。
        best_html = None
        best_url = None
        ...
        for idx, candidate_page in enumerate(
            ...
            try:
                ...
                print()
                print(
                    f"手動確認タブ候補 {idx}/{len(matching_pages)}: {current_url}"
                )

                await candidate_page.wait_for_timeout(
                    500
                )

                dom_html = await candidate_page.content()
        ...
        if (
            ...
            print()
            print(
                f"[成功] 手動表示されたChrome DOMからHTMLを取得しました。"
            )
            print(
                f"採用HTML文字数: {len(best_html):,}"
            )

        elif best_html is not None:

            print()
            print(
                "[警告] 読み取れたDOM HTMLが短すぎます。"
            )
            print(
                "403画面やエラーページの可能性があります。"
            )
